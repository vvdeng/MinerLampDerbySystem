...
class LoginDialog extends JDialog {
	public LoginDialog(final MinerLampFrame owner) {
		...
		panel.add(
				new JLabel(new ImageIcon("resources/login.png")),
				new GBC(0, 0, GridBagConstraints.REMAINDER, 1).setFill(
						GridBagConstraints.HORIZONTAL).setWeight(100, 0));

		panel.add(new JLabel("用户名："), new GBC(0, 1).setWeight(100, 0)
				.setAnchor(GridBagConstraints.EAST));
		panel.add(new JLabel("  密码："), new GBC(0, 2).setWeight(100, 0)
				.setAnchor(GridBagConstraints.EAST));
		...
		ActionListener loginActionListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				User currentUser = userDAO.checkLogin(nameTxt.getText(),
						pwdTxt.getText());
				if (currentUser != null) {
					...
					SysInf regist = GlobalData.sysInfoMap
							.get(SysInf.CAT_USER_REGIST);
					SysInf usedCountInf = GlobalData.sysInfoMap
							.get(SysInf.CAT_USED_COUNT);
					if (regist.getVal().equals(SysInf.VAL_TRY_COUNT)) {
