...
public class TableUtil {
	...
	public static TableModel getModel(final List list,
			final String columnNames[], final Set<Integer> dateTypeSet) {
		...
		AbstractTableModel tm = new AbstractTableModel() {
			...
			public Object getValueAt(int row, int column) {
				...
				/*
				 * if (Date.class.isInstance(object)) { return
				 * Util.formatDateTime((Date) object); }
				 */
				if (dateTypeSet.contains(column)) {
					if (object != null&&(Long)object!=0L) {
						return Util.formatDateTime(new Date((Long) object));
					}
				}
				return object;
			}
